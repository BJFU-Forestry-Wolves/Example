...
//		Chassis_StatusTypeDef *chassis_status = Chassis_StatusPtr();		
		...
		for(int i = 0 ; i < 4 ; i++ )
    {
		...
		Motor_SetMotorOutput(&Motor_ChassisFontRightMotor, set_output1);
		 

    Motor_SetMotorOutput(&Motor_ChassisFontLeftMotor, set_output2);		
		

    Motor_SetMotorOutput(&Motor_ChassisBackLeftMotor, set_output3);


    Motor_SetMotorOutput(&Motor_ChassisBackRightMotor, set_output4);
}
